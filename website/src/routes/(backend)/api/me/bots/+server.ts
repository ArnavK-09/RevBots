...
export const GET = (async ({ cookies }) => {
	...
	// if
	if (!auth) {
		throw error(401, { message: 'No auth provided' });
	}
...
export const POST = (async ({ request, cookies }) => {
	...
	// fetch username
	const bot: AxiosResponse = await axios
	...
	const revoltBotData: AxiosResponse = await axios
	...
	const botData: any = {
		...
		discriminator: revoltBotData.data.discriminator,
